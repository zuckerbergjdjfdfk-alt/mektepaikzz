...
create table public.staff (
  ...
  role text not null, -- director, vice_principal, teacher, janitor, handyman, secretary, nurse
...
create table public.classes (
  id uuid primary key default gen_random_uuid(),
  name text not null, -- "1А", "2Б"
  grade int not null,
  letter text not null,
  student_count int default 0,
  parallel text generated always as (grade::text) stored,
  created_at timestamptz default now()
);
...
create table public.rooms (
  ...
  type text default 'standard', -- standard, gym, chemistry, language_lab, music
  created_at timestamptz default now()
);
...
create table public.subjects (
  ...
  is_lenta boolean default false -- предмет с лентами (английский по уровням)
);

-- Учебная нагрузка: сколько часов какому классу по предмету в неделю
...
create table public.schedule_slots (
  id uuid primary key default gen_random_uuid(),
  day_of_week int not null, -- 1-6 (пн-сб)
  period int not null, -- 1-8 урок
  class_id uuid references public.classes(id) on delete cascade,
  subject_id uuid references public.subjects(id),
  teacher_id uuid references public.staff(id),
  room_id uuid references public.rooms(id),
  lenta_group text, -- для лент: "english_beginner", "english_intermediate"
...
create table public.tasks (
  ...
  source text default 'manual', -- manual, voice, telegram, whatsapp, ai
  source_message text,
  created_at timestamptz default now(),
...
create table public.incidents (
  ...
  location text,
  reported_by text,
  source text default 'telegram', -- telegram, whatsapp, manual
  source_message text,
...
-- Посещаемость (по классам, ежедневная)
...
create table public.nfc_scans (
  ...
  scan_type text default 'entry' -- entry, exit, cafeteria
);

-- Сообщения чатов (TG + WA)
create table public.chat_messages (
  ...
  sender_name text,
  content text,
  parsed_intent text, -- attendance, incident, task, other
  parsed_data jsonb,
  raw jsonb,
  created_at timestamptz default now()
);
...
create table public.order_templates (
  id uuid primary key default gen_random_uuid(),
  code text not null unique, -- "MZ-76", "MON-130"
  title text not null,
  category text, -- health, hr, education, internal
  description text,
...
create table public.notifications (
  id uuid primary key default gen_random_uuid(),
  type text not null, -- attendance_summary, incident, task, schedule_conflict
  title text not null,
  body text,
  payload jsonb,
...
-- RLS: на этапе MVP открываем чтение/запись всем (демо без авторизации)
...
-- Демо-политики: открыто всем (для хакатона)
do $$ declare t text;
begin
  for t in select unnest(array['staff','classes','rooms','subjects','curriculum','schedule_slots','tasks','incidents','attendance','nfc_scans','chat_messages','order_templates','generated_orders','notifications']) loop
    ...
    execute format('create policy "demo_delete" on public.%I for delete using (true);', t);
  end loop;
end $$;
